...
def creation(indice_classe, mode, num_quest, titre, l_case_fichier):
    ...
    for j in range(len(l_res)):
        ...
        for i in l_indice:
            temp_res = str(num_quest) + ". " + res[i] + "\n\n"
            temp_rep = str(num_quest) + "\t" + \
                l_rep[j][i].replace("\n", "") + "\n"
...
quitter = tk.Button(main, text="Quitter",
                    command=main.destroy)  # Bouton Quitter
...
case_classe.grid(row=1, column=3, rowspan=5, sticky='nesw')
...
compt = 1
canv_quest = tk.Canvas(main, width=320, height=400,
                       scrollregion=(0, 0, 700, len(l_types)*38))
cont_quest = tk.Frame(canv_quest)
canv_quest.grid(row=1, rowspan=6, column=5)
defilY_quest = tk.Scrollbar(main, orient='vertical',
                            command=canv_quest.yview)
defilY_quest.grid(row=1, rowspan=6, column=6, sticky='ns')
canv_quest['yscrollcommand'] = defilY_quest.set
defilX_cr = tk.Scrollbar(main, orient='horizontal',
                         command=canv_quest.xview)
defilX_cr.grid(row=7, column=5, sticky='ew')
canv_quest['xscrollcommand'] = defilX_cr.set
for i in range(len(l_types)):
    fichier = l_types[i]
    nb_questions = tk.StringVar()
    nb_questions.set("0")
    l_case_fichier.append([tk.Label(cont_quest, textvariable=l_q_rest[i]), tk.Entry(
        cont_quest, textvariable=nb_questions, width=6), nb_questions])
    l_case_fichier[-1][0].grid(row=compt, column=0)
...
case_choix_creer = tk.Label(
    main, text="Créer un nouveau QCM ou compléter celui existant ?")
...
bouton_start = tk.Button(main, text="Créer le QCM", state='normal', command=lambda: creation(case_classe.curselection(
), mode.get(), int(num_quest.get()), titre.get(), l_case_fichier))  # Bouton pour lancer la création du QCM
...
compte_rendu.set(" "*30)
canv_cr = tk.Canvas(main, width=250, height=400,
                    scrollregion=(0, 0, 500, 2000))
canv_cr.grid(row=1, rowspan=6, column=5)
defilY_cr = tk.Scrollbar(main, orient='vertical',
                         command=canv_cr.yview)
defilY_cr.grid(row=1, rowspan=6, column=9, sticky='ns')
canv_cr['yscrollcommand'] = defilY_cr.set
defilX_cr = tk.Scrollbar(main, orient='horizontal',
                         command=canv_cr.xview)
defilX_cr.grid(row=7, column=8, sticky='ew')
...
canv_cr.grid(row=1, rowspan=5, column=8)
...
case_reset_data = tk.Button(main, text="Réinitialiser les données\nde la classe sélectionnée.",
                            command=lambda: supp_data(l_classes[case_classe.curselection()[0]], False))
# Case pour reset les données d'une classe
case_reset_data.grid(column=3, row=6)

# Case pour créer une classe
case_crea_classe = tk.Label(main, text="Nouvelle classe")
...
bouton_verif_data = tk.Button(
    main, text="Lancer l'analyse", command=lambda: verification_databank())
bouton_verif_data.grid(row=4, column=4)

case_shuffle = tk.Label(main, text="Mélanger les questions")
case_shuffle.grid(row=5, column=4)
shuffle_state = tk.IntVar()
check_shuffle = tk.Checkbutton(main, text="Mélange", variable=shuffle_state)
check_shuffle.grid(row=6, column=4)
