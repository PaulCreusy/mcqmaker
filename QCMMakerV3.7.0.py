...
def convert_letter_number(letter):
    letter = letter.replace(" ", "")
    letter = letter.replace("\n", "")
    letter = letter.replace("\t", "")
    try:
        return True, ord(letter) - 64
    except:
        return False, letter
...
def creation(fichier_data, mode, num_quest, titre, l_case_fichier):
    ...
    for i in range(nb_type):
        ...
        for j in range(nb_ligne):  # Dans cette boucle on ajoute les questions une par une
            if j in id_questions:
                ...
                ligne, rep = ligne.split(" @ ")
                ligne += "\n"
                ligne_exp = ligne.replace("\n", "")
                l_rep[num_dossier].append(rep)
                ligne = ligne.split(" : ")
                ligne_exp = ligne_exp.split(" : ")
                el_exp = [ligne_exp[0]]
                sucess, id_rep = convert_letter_number(rep)
                if not(sucess):
                    id_rep = 1
                    el_exp = [ligne_exp[0] + " #ERREUR#"]
                    compte_rendu.set(compte_rendu.get() + "\n" +
                                     "Erreur lors de la lecture de la réponse")
                el_exp.append(ligne_exp[id_rep])
                for i in range(1, len(ligne_exp)):
                    if i != id_rep:
                        el_exp.append(ligne_exp[i])
                print(el_exp)
                l_exp.append(el_exp)
                question = ligne[0]
                l_reponses = [" "] * (len(ligne) - 1)
    ...
    print(l_exp)
    print(l_finale)
    l_order_indice = [i for i in range(len(l_finale))]
    if shuffle_state.get() == 1:
        shuffle(l_order_indice)
    l_finale = [l_finale[i] for i in l_order_indice]
    l_exp = [l_exp[i] for i in l_order_indice]

    cur_exp = open("export_" + titre + ".txt", mode="w")
