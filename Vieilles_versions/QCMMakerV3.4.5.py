...
def creation(indice_classe, mode, num_quest, titre, l_case_fichier):
    global state, compte_rendu
    compte_rendu.set(" "*30)
    if indice_classe == ():
        compte_rendu.set("Erreur: aucune classe sélectionnée")
        return
    ...
    cur_data = open("Classes/" + fichier_data,"r",-1,"utf-8") # Ouverture du fichier data

    nom_classe = cur_data.readline()

    compte_rendu.set(compte_rendu.get() + "\nClasse choisie: " + fichier_data.replace(".txt",""))
    compte_rendu.set(compte_rendu.get() + "\nmode choisi: " + mode)
    ...
    for ligne in cur_data:
        ...
        for i in range(nb_type):
            if type == plan[i][0]:
                plan[i][2] = dejavu # On charge dans plan la liste des questions déjà utilisées
    ...
    for i in range(nb_type):
        ...
        if data_local[1] > nb_indice: # On évite les erreurs liées à l'utilisation de toutes les question de la base
            choix_erreur = askyesno("Erreur", "Erreur: plus assez de questions non utilisées\ndans la base pour terminer le QCM. Souhaitez-vous\ncontinuer malgré tout? Le nombre de questions dans\nle QCM sera inférieur au nombre de questions\ndemandé pour cette catégorie: " + data_local[0], icon = "warning")
            if choix_erreur:
                data_local[1] = nb_indice
            else:
                compte_rendu.set(compte_rendu.get() + "\n" + "Annulé")
                return
        id_questions = sample(l_indices,data_local[1]) # On sélectionne aléatoirement les questions du QCM
    ...
    for j in range(len(l_res)):
        ...
        for i in l_indice:
            ...
            cur_rep.write(str(num_quest) + "\t" + l_rep[j][i].replace("\n","") + "\n")
            num_quest += 1
    ...
    cur_res.close()
    cur_rep.close()
    cur_data = open("Classes/" + fichier_data,"w",-1,"utf-8") # On le rouvre pour y écrire les nouvelles données
...
def update_nb_total_quest():
    ...
    nb_total_quest.set("Nombre total de questions: " + str(sum([int(l_case_fichier[i][2].get()) for i in range(len(l_case_fichier))])))
...
def update_quest_rest():
    global l_q_rest
    d_q_rest = {}
    if case_classe.curselection() == ():
        return
    classe_choisie = l_classes[case_classe.curselection()[0]]
    ...
    for i in range(len(l_types)):
        fichier = l_types[i]
        l_q_rest[i].set(fichier + "\nQuestions restantes: " + str(d_q_rest[fichier]))
...
quitter = tk.Button(main, text = "Quitter", command = main.destroy) # Bouton Quitter
...
case_choix_classe.grid(row = 0, column = 3)
chaine =""
for classe in l_classes:
    chaine += " " + classe.replace(".txt","")
liste_classe = tk.StringVar()
liste_classe.set(chaine)
case_classe = tk.Listbox(main, listvariable = liste_classe)
case_classe.selection_set(0)
case_classe.grid(row = 1, column = 3, rowspan = 5)

case_banque_donn = tk.Label(main, text = "Nombre de questions par fichier:") # Haut de colonne pour banque
case_banque_donn.grid(row = 0, column = 5, columnspan = 2)
...
for i in range(len(l_types)):
    ...
    nb_questions.set("0")
    l_case_fichier.append([tk.Label(main, textvariable = l_q_rest[i]),tk.Entry(main, textvariable = nb_questions),nb_questions])
    l_case_fichier[-1][0].grid(row = compt, column = 5)
    l_case_fichier[-1][1].grid(row = compt, column = 6)
    compt += 1

case_choix_creer = tk.Label(main, text = "Créer un nouveau QCM ou compléter celui existant ?")
case_choix_creer.grid(row = 3, column = 0, columnspan = 2)
...
case_num_prem_quest = tk.Label(main, text = "Numéro de la 1ère question:") # Case pour le numéro de la 1ère question
...
bouton_start = tk.Button(main, text = "Créer le QCM", state =  'normal', command = lambda: creation(case_classe.curselection(), mode.get(), int(num_quest.get()), titre.get(), l_case_fichier)) # Bouton pour lancer la création du QCM
bouton_start.grid(row = 5, column = 0)
state = tk.StringVar()
case_state = tk.Label(main, textvariable = state)
case_state.grid(row = 6, column = 1)

compte_rendu.set(" "*30)
case_compte_rendu = tk.Message(main, textvariable = compte_rendu)
case_compte_rendu.grid(row = 1, rowspan = 5,column = 8) # Case de compte rendu des actions
case_nom_compte_rendu = tk.Label(main, text = "Résumé des actions effectuées:")
case_nom_compte_rendu.grid(row = 0, column = 8)

case_reset_data = tk.Button(main, text = "Réinitialiser les données\nde la classe sélectionnée.", command = lambda: supp_data(l_classes[case_classe.curselection()[0]],False))
case_reset_data.grid(column = 3, row = 6) # Case pour reset les données d'une classe

case_crea_classe = tk.Label(main, text = "Nouvelle classe") # Case pour créer une classe
case_crea_classe.grid(row = 0, column = 4)
...
case_nom_nouv_classe.grid(row = 1, column = 4)
bouton_crea_classe = tk.Button(main, text = "Créer une nouvelle classe", command = lambda: creation_classe(nom_nouv_classe.get()))
bouton_crea_classe.grid(row = 2, column = 4)

bouton_crea_brut = tk.Button(main, text = "Créer un QCM à partir\ndu fichier brut", command = lambda: creation_brut(titre.get(),int(num_quest.get()),mode.get()))
bouton_crea_brut.grid(row = 5, column = 1) # Bouton pour lancer QCMMaker V1

case_verif_data = tk.Label(main, text = "Vérification de la base de questions")
case_verif_data.grid(row = 3, column = 4)
bouton_verif_data = tk.Button(main, text = "Lancer l'analyse", command = lambda: verification_databank())
bouton_verif_data.grid(row = 4, column = 4)
...
case_nb_total_quest.grid(row = 6, column = 1)
